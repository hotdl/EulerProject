...
numList = [int(x) for x in numStr.strip().replace("\n", " ").split(" ")]
row = int((len(numList)*2)**0.5)
for n in range(row-1, 0, -1):
    for i in range(n):
        curIndex = n*(n-1)/2+i
        leftChildIndex = (n+1)*n/2+i
        rightChildIndex = (n+1)*n/2+1+i
        numList[curIndex] += max(numList[leftChildIndex], numList[rightChildIndex])
print numList[0]
